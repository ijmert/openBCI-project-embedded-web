...
queueName = "packageUploadQueue"
ipaddress = "192.168.0.236"
connection = pika.BlockingConnection(pika.ConnectionParameters(host=ipaddress, virtual_host="openbci", credentials=pika.PlainCredentials('ijmertadmin', 'ijmertadmin')))
...
def publishmessage(channel, packageString):
    channel.basic_publish(exchange='', routing_key=queueName, body=packageString)
        ...
        while True:
            if len(unprocessed_data[0]) > sampleNumbers:
                ...
                if len(dP.Channels[0]) != sampleNumbers:
                    ...
                    serializedDP = json.dumps(dP.__dict__, indent=4)
                    publishmessage(channel, serializedDP)
